...
@section('content')
    <div class="page-body">
        ...
        <div class="container-fluid">
            ...
            @if($status == 0)
                <button class="btn btn-success btn-md mr-2 startInningButton">Start 1st Inning</button>
            ...
            @elseif($status == 4)
                ...
                <h5 class="mt-5">{{ $game->WON ? $game->WON->team_name : NULL}} {{$game->description}}</h5>

                @if($game->mom != NULL && $game->mom != '--')
                    <h5 class="mt-5">Man of the Match
                        : {{ $game->MOM ? $game->MOM['first_name'] : NULL}} {{$game->MOM ? $game->MOM['last_name'] : NULL}}</h5>
                @endif
                @if($game->WON)
